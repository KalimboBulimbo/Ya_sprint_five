...
func (t Training) distance() float64 {
	...
	trainDist := (float64(t.Action) * t.LenStep / float64(MInKm))
...
func (t Training) meanSpeed() float64 {
	...
	trainSpeed := float64(t.distance()) / t.Duration.Hours()
...
func (t Training) TrainingInfo() InfoMessage {
	...
	infoT := InfoMessage{
		...
		Speed:        t.meanSpeed(),  // float64     // средняя скорость, с которой двигался пользователь
		Calories:     t.Calories(),   // float64
	}
	return infoT
...
func (r Running) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	infoR := InfoMessage{
		TrainingType: r.Training.TrainingType, // тип тренировки
		Duration:     r.Training.Duration,     // длительность тренировки
		Distance:     r.Training.distance(),   // расстояние, которое преодолел пользователь
		Speed:        r.Training.meanSpeed(),  // float64     // средняя скорость, с которой двигался пользователь
		Calories:     r.Training.Calories(),   // float64
	}
	return infoR
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	//получение квадрата скорости в м/с
	speedSquareMS := math.Pow(w.meanSpeed()*KmHInMsec, 2)
	//получение роста в метрах
	heightInM := w.Height /// CmInM

	spentCalories := ((CaloriesWeightMultiplier*w.Weight + (speedSquareMS/heightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	//durationInH := float64(s.Duration / MinInHours)
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	durationInH := float64(s.Duration / MinInHours)
	return (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * durationInH
